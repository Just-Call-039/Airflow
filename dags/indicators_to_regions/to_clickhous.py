...
# Выгружаем датафрейм с рабочими часами
 ...
 workhour['total_sec'] = workhour.apply(lambda row: defs.total_sec(row), axis=1)

# Преобразуем и переименуем данные для дальшнейшего мерджа
 workhour = workhour[['id_user', 'date', 'total_sec']].rename(columns={'id_user': 'userid', 'date' : 'datecall'})
 df.loc[df['datecall'] == '0','datecall'] = ''
 df['datecall'] = df['datecall'].astype('datetime64').dt.to_period('D').fillna('').astype('str').fillna('')
 
 df = df.merge(workhour, how = 'left', on = ['userid', 'datecall'])
...
# Подтянем в датасет значения регионов по пользователю
 ...
 df.loc[df['РТК регион'] == 'Юг', 'РТК регион'] = 'ЮГ'
 ...
 try:
    ...
    client.execute('truncate table suitecrm_robot_ch.indicators_to_region')
 except (ValueError):
    ...
    try:
        ...
        sql_create = '''create table if not exists indicators_to_region
            (
            Дата                        Date,
            ...
            engine = MergeTree ORDER BY Дата;'''
...
def call_union(path_to_file, csv_city, path_to_request, path_to_call, path_to_transfer, path_to_user, 
               path_to_decoding, \
                path_to_workhour, path_to_result, file_name_result):
...
# Звонки
 print('___start read call')
 call = pd.read_csv(path_to_call, sep=';')
 print(call.columns)
...
# Переводы
 print('___start read transfer')
 transfer = pd.read_csv(path_to_transfer, sep=';')
 print('___download transfer done')
 print('transfer ', transfer.shape[0])
...
# Убираем нули, преобразуем типы данных
 ...
 call['datecall'] = call['datecall'].astype("datetime64[ns]").dt.to_period("D").fillna('').astype(str)
...
# Поменяем типы данных для мерджа
 ...
 transfer['datecall'] = transfer['datecalls'].fillna('').astype(str)
...
#  Удаляем дубликаты
 ...
 dict_city = pd.read_csv(f'{path_to_file}{csv_city}')
 del dict_city['Название из JSON карты']
...
# Опеределим регионы проектов
 calltown['RTK_region'] = calltown.apply(lambda row: defs.region_defination(row['RTK_city'],
                                                                                 ...
                                                                                 row['RTK_town']), axis=1)
 print('__РТК region complete')
 print('Уникальные значения регионов RTK ', calltown['RTK_region'].unique())
 print()
 calltown['TTK_region'] = calltown.apply(lambda row: defs.region_defination(row['TTK_city'], 
                                                                                 row['TTK_city_c'],
                                                                                 row['TTK_town']), axis=1) 
 print('__ТТК region complete')
 print('Уникальные значения регионов TTK ', calltown['TTK_region'].unique())
 calltown['MTS_region'] = calltown.apply(lambda row: defs.region_defination(row['MTS_city'], 
                                                                                 row['MTS_city_c'],
                                                                                 row['MTS_town']), axis=1) 
 print('__МТС region complete')
 print('Уникальные значения регионов MTS ', calltown['MTS_region'].unique())
 calltown['BLN_region'] = calltown.apply(lambda row: defs.region_defination(row['BLN_city'], 
                                                                                 row['BLN_city_c'],
                                                                                 row['BLN_town']), axis=1) 
 print('__BLN region complete')
 print('Уникальные значения регионов BLN ', calltown['BLN_region'].unique())
...
#  Подтянем значения причин отказов из справочника
 ...
 callotkaz =  calltown.merge(df_decoding_otkaz, on = 'otkaz_c', how = 'left')
 print('___size df callotkaz ', callotkaz.shape[0])
...
# merge с лидами
 print('___merge call_group & lids')
 call_lids =  call_user.merge(lids[['Проект','СВ CRM', 'МРФ']], left_on = 'supervisor', right_on = 'СВ CRM', how = 'left')
 print('__size df ', call_lids.shape[0])

# merge с проектами
 print('___merge call_group & jc')
 call_jc =  call_lids.merge(jc[['Проект','CRM СВ']], left_on = 'supervisor', right_on = 'CRM СВ', how = 'left')
 print('___size df ', call_jc.shape[0])
...
# Загружаем заявки
 ...
 request = pd.read_csv(path_to_request, sep=';').fillna('')
...
#  Убирем лишний ноль из телефона
 request['contact'] = request['contact'].astype(str)
 defs.del_point_zero(request, ['contact'])

# Меняем типы данных для дальнейшего merge
 request['dateentered'] = pd.to_datetime(request['dateentered'])
 request = request[(request['dateentered'] >= '2024-02-01') & (request['dateentered'] <= pd.to_datetime('today'))]

 print('___size request ', request.shape[0])
 request['dateentered']=request['dateentered'].fillna('').astype('str')
 request['contact']=request['contact'].fillna('').astype('str')

#  Переименуем столбцы, чтобы не дублировались
 request = request[['dateentered', 'userid', 'statused', 'queue','contact']].\
                                                            rename(columns = {'statused' : 'status_request',
                                                                              'queue' : 'queue_request',
                                                                              'userid' : 'user'                                                          
                                                                             })
...
# Так же для заявок. Будем мерджить по 1 - самая первая заявка за день на этот номер
# Все заявки попадут в датасет, но не продублируют звонки
 request = request.sort_values(['dateentered', 'user', 'contact'])    
 request['NN'] = request.groupby(['dateentered', 'user', 'contact']).cumcount() + 1

# Объединяем датафрейм с запросами
 print('start merge df_request')
 df =  call_jc.merge(request, left_on = ['phone', 'userid', 'datecall', 'NN'],
                                right_on = ['contact', 'user', 'dateentered', 'NN'], how = 'outer')
...
# Вернем столбцам с датами тип дата 
 ...
 print('кол-во звонков', df['otkaz_c'].astype('int64').sum())
 print('кол-во заявок в датасете', df[df['dateentered'] != '0'].shape[0])

# Выгружаем датафрейм с рабочими часами
 
 workhour = pd.read_csv(path_to_workhour, sep = ';')
 print('size work', workhour.shape[0])
...
# Преобразуем и переименуем данные для дальшнейшего мерджа
 ...
 df['datecall'] = df['datecall'].astype('datetime64').dt.to_period('D').fillna('').astype('str')

# Смерджим датасет с рабочими часами 
 df = df.merge(workhour, how = 'left', on = ['userid', 'datecall'])
 print('size df', df.shape[0])
 print(df['total_sec'].unique())
 df['datecall'] = df['datecall'].fillna('')
...
# Переименуем и оставим только нужные столбцы
 ...
 df = df[['datecall',
         ...
         'status_request', 
         'queue_request' 
         ]].rename(columns={'datecall' : 'date_call',
                           'userid' : 'user_id', 
                           'phone' : 'phone',
                           'town' : 'town_c', 
                           ...
                           'total_sec' : 'work_hour',
                           'otkaz_c' : 'count_call',
                           'dateentered' : 'date_request',
                           'user' : 'user_request',
                           'contact' : 'request_phone',
                           'status_request' : 'status_request', 
                           'queue_request' :'queue_request' })

# Подтянем в датасет значения регионов по пользователю
 
 lids = lids.rename(columns={'СВ CRM': 'supervisor', 'Проект' : 'project_supervisor','МРФ' : 'MRF'})
 
 df = df.merge(lids[['supervisor', 'project_supervisor', 'MRF']], how = 'left', on = 'supervisor')

 df['count_call'] = df['count_call'].fillna('0').astype('str') 
 df['work_hour'] = df['work_hour'].fillna('0').astype('str')
 df['queue_request'] = df['queue_request'].fillna('0').astype('str')

 col_list = ['work_hour', 'count_call', 'queue_request']
 defs.del_point_zero(df, col_list)

 df.loc[df['date_request'] == '0','date_request'] = ''
 df.loc[df['date_call'] == '0', 'date_call'] = ''
 df.loc[df['RTK_region'] == 'Юг', 'RTK_region'] = 'ЮГ'

# Сохраняем csv в папку

 df.to_csv(f'{path_to_result}{file_name_result}', index = False)

def call_union_1(path_to_file, csv_city, path_to_request, path_to_call, path_to_transfer, path_to_user, 
               path_to_decoding, \
                path_to_workhour, path_to_result, file_name_result):
...
#  Удаляем дубликаты
 ...
 dict_city = pd.read_csv(csv_city)
 del dict_city['Название из JSON карты']
...
#  request['NN'] = request.groupby(['dateentered', 'user', 'contact']).cumcount() + 1
 ...
 request = request.groupby(['dateentered', 'user', 'contact', 'status_request'])[['count']].sum().reset_index()
 request = request.rename(columns={'count' : 'count_request'})

# Объединяем датафрейм с запросами
 print('start merge df_request')
 df =  call_jc.merge(request, left_on = ['phone', 'userid', 'datecall'],
                                right_on = ['contact', 'user', 'dateentered'], how = 'outer')
...
def to_click_previous(path_to_result, file_name):
 
#  Загружаем датафрейм
 df = pd.read_csv(f'{path_to_result}{file_name}')
...
# # Очищаем таблицу indicators_to_region

#  print('delete dates from table indicators_to_region_new')
#  client.execute('''delete from suitecrm_robot_ch.indicators_to_region_previous
#                  where DATE_FORMAT(`Дата`, '%Y-%m') = DATE_FORMAT({}, '%Y-%m')'''.format(n))

 
#  client = Client(host=host, port='9000', user=user, password=password,
#                     database='suitecrm_robot_ch', settings={'use_numpy': True})

# # Создаем таблицу  indicators_to_region_previous

#  print('Create table indicators_to_region_previous')
#  sql_create = '''create table suitecrm_robot_ch.indicators_to_region_previous
#     (
#     date_call                   Date,
#     dialog                      String,
#     user_id                     String,            
...
#     MRF                         String
#     )
#     engine = MergeTree ORDER BY date_call;'''
#  client.execute(sql_create)
...
# Записываем новый данные в таблицу usermetric_call
    ...
    print('insert table indicators_to_region_previous')

    client.insert_dataframe('INSERT INTO suitecrm_robot_ch.indicators_to_region_previous VALUES', df)
 except (ValueError):
    print('Данные не загружены')
 finally:
...
# def to_click_previous_1(path_to_result, file_name):
...
#     print('Create table indicators_to_region_previous')
#     sql_create = '''create table suitecrm_robot_ch.indicators_previous_test
#         (
#         date_call                   Date,
#         dialog                      String,
#         user_id                     String,            
...
#     print('insert table indicators_previous_test')

#     client.insert_dataframe('INSERT INTO suitecrm_robot_ch.indicators_previous_test VALUES', df)
#  except (ValueError):
#     print('Данные не загружены')
#  finally:
...
def to_click_current(path_to_result, file_name):
...
#  Загружаем датафрейм
 ...
 try:
    ...
    # # Удаляем таблицу indicators_to_region

    print('drop table indicators_to_region_current')
    client.execute('''truncate table suitecrm_robot_ch.indicators_to_region_current''')
 except (ValueError):
    print('Данные не удалены')
 finally:
    try:
        ...
        # Создаем таблицу  indicators_to_region_current

        print('Create table indicators_to_region')
        sql_create = '''create table if not exists suitecrm_robot_ch.indicators_to_region_current
            (
            date_call                   Date,
            dialog                      String,
            user_id                     String,            
        ...
        try:
            # Записываем новый данные в таблицу usermetric_call
            print('insert table indicators_to_region')
            client.insert_dataframe('INSERT INTO suitecrm_robot_ch.indicators_to_region_current VALUES', df)
        except (ValueError):
            print('Данные не загружены')
        finally:
...
# def to_click_current_1(path_to_result, file_name):
...
#     print('drop table indicators_current_test')
#     client.execute('''truncate table suitecrm_robot_ch.indicators_current_test''')
#  except (ValueError):
#     print('Данные не удалены')
#  finally:
#     try:
...
#         print('Create table indicators_to_region')
#         sql_create = '''create table if not exists suitecrm_robot_ch.indicators_current_test
#             (
#             date_call                   Date,
#             dialog                      String,
#             user_id                     String,            
...
#             # Записываем новый данные в таблицу usermetric_call
#             print('insert table indicators_to_region')
#             client.insert_dataframe('INSERT INTO suitecrm_robot_ch.indicators_current_test VALUES', df)
#         except (ValueError):
#             print('Данные не загружены')
#         finally:
...
# def test_to_click(path_to_result, file_name):
#     #  Загружаем датафрейм
#  df = pd.read_csv(f'{path_to_result}{file_name}')
...
#     # # Удаляем таблицу indicators_to_region_test

#     print('drop table indicators_to_region')
#     client.execute('''truncate table suitecrm_robot_ch.indicators_to_region_test''')
#  except (ValueError):
#     print('Данные не удалены')
#  finally:
#     try:

#         # Создаем таблицу  indicators_to_region_test

#         print('Create table indicators_to_region_test')
#         sql_create = '''create table if not exists suitecrm_robot_ch.indicators_to_region_test
#             (
#             date_call                   Date,
#             dialog                      String,
#             user_id                     String,            
...
#             )
#             engine = MergeTree
#             PARTITION BY toYYYYMMDD(date_call)
#             ORDER BY date_call;'''
#         client.execute(sql_create)
#     except (ValueError):
#         print('Таблица не создана')
...
#             # Записываем новый данные в таблицу indicators_to_region_test
#             print('insert table indicators_to_region_test')
#             client.insert_dataframe('INSERT INTO suitecrm_robot_ch.indicators_to_region_test VALUES', df)
#         except (ValueError):
#             print('Таблица не создана')
#         finally:
#             client.connection.disconnect()
