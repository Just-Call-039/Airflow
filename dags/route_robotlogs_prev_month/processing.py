...
import pandas as pd
import route_robotlogs_prev_month.all_functions as func
from route_robotlogs_prev_month.sql_query import sql_query_to_csv, sql_query_to_csv_log
import numpy as np
from datetime import datetime, date


def group_log(path_project_folder,
            path_sql_log,
            path_sql_dest,
            path_sql_city,
            name_df_step,
    ...
    file_name_log = 'robot_log_{f}'.format(f=i_month)
    file_name_route = 'route_{f}'.format(f=i_month)
    file_name_route_unique = 'route_unique_{f}'.format(f=i_month)
    file_name_city = 'city_{f}'.format(f=i_month)
    file_name_dest = 'dest_{f}'.format(f=i_month)

    # Загрузим датасеты с шагами, командами и очередями
        
    steps = pd.read_csv(f'{path_project_folder}{name_df_step}')
    ...
    sql_query_to_csv_log(cloud=cloud_name, path_sql_file=path_sql_log, date_f=date_f, i_month=i_month, path_csv_file=path_to_robot_log, name_csv_file=file_name_log)
    sql_query_to_csv(cloud=cloud_name, path_sql_file=path_sql_city, date_f=date_f, path_csv_file=path_project_folder, name_csv_file=file_name_city)
    sql_query_to_csv(cloud=cloud_name, path_sql_file=path_sql_dest, date_f=date_f, path_csv_file=path_project_folder, name_csv_file=file_name_dest)
