...
def group_log(path_project_folder, path_sql_log, path_sql_dest, path_sql_city, path_to_robot_log,
              path_to_defenition, path_to_route, path_to_route_unique, file_name_operator):
    ...
    for i in range(1, 6):
        ...
        file_name_log = 'robot_log_{f}.csv'.format(f=i)
        file_name_city = 'city_{f}.csv'.format(f=i)
        file_name_dest = 'dest_{f}.csv'.format(f=i)
        file_name_route = 'route_{f}.csv'.format(f=i)
        file_name_route_unique = 'route_unique_{f}.csv'.format(f=i)
