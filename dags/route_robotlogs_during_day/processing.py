...
from route_robotlogs_during_day.sql_query import sql_query_to_csv
import numpy as np
import datetime


def group_log(path_project_folder, path_sql_log, path_sql_dest, path_sql_city, path_to_robot_log,
              path_to_defenition, name_df_step, name_df_team, name_df_queue, file_name_route, file_name_route_unique, 
              file_name_log, file_name_city, file_name_dest,
              file_name_operator, path_to_route, path_to_route_unique):
    ...
    teams = pd.read_csv(f'{path_project_folder}{name_df_team}')
    print('___download teams', teams.shape[0])
    queue_project = pd.read_csv(f'{path_project_folder}{name_df_queue}')
    print('___download queue', queue_project.shape[0])
    
    #  Загружаем датасеты с логами, названными городами и переводами

    sql_query_to_csv(cloud=cloud_name, path_sql_file=path_sql_log, path_csv_file=path_to_robot_log, name_csv_file=file_name_log)
    sql_query_to_csv(cloud=cloud_name, path_sql_file=path_sql_city, path_csv_file=path_project_folder, name_csv_file=file_name_city)
    sql_query_to_csv(cloud=cloud_name, path_sql_file=path_sql_dest, path_csv_file=path_project_folder, name_csv_file=file_name_dest)
    ...
    new_df = df_union[['phone', 'assigned_user_id', 'date', 'last_step', 'client_status', 'quality',
                ...
                'city_c', 'call_city', 'was_repeat', 'trunk_id', 'inbound_call', 'transfer', 'project']]
    ...
    df_group = new_df.groupby(['assigned_user_id', 'date', 'city_c', 'queue', 'destination_queue', 
                        ...
                        'call_city', 'trunk_id', 'inbound_call', 'transfer', 'project', 
                        'route'])[['phone', 'billsec', 'real_billsec']]\
    ...
    print('___df route unique size ', route_unique.shape[0])
    ...
    print('___df route size ', route_unique.shape[0])
    ...
    df_group = df_group[['date', 'call_city', 'project', 'queue', 'destination_queue', 'result',
                    ...
                    'trunk_id', 'phone', 'inbound_call', 'transfer']]
