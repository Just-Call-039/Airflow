with callss as (select asterisk_caller_id_c, date(date_entered) date_entered, result_call_c, duration_minutes
                from calls
                         left join calls_cstm on id = id_c
                where date(date_entered) = date(now()))

select caller_id                phone,
       date(call_time) calldate,
...
from waiter_log
         left join suitecrm_robot.jc_robot_log on phone = caller_id and date(call_date) = date(call_time)
         ...
         left join callss on caller_id = asterisk_caller_id_c
where date(call_date) = date(now()) and (result_call_c is null or duration_minutes <10)